...
class EGCSR_BS_Clustering:
    ...
    def __adjacent_mat(self, x, n_neighbors=10):   #k近邻算法来求得邻接矩阵
        """
    ...
    def post_proC(self, C, K, d, alpha):
        ...
        C = 0.5 * (C + C.T)
        r = d * K + 1
        U, S, _ = svds(C, r, v0=np.ones(C.shape[0]))  #奇异值分解
        #U, S, _ = la.svd(C)  #奇异值分解
        U = U[:, ::-1]
        S = np.sqrt(S[::-1])
        S = np.diag(S)
        U = U.dot(S)
        ...
        Z = U.dot(U.T)  
        Z = Z * (Z > 0)
        L = np.abs(Z ** alpha)  #得到亲和矩阵
        '''
        sns.set()
        ax = sns.heatmap(L,cmap='rainbow')
        plt.show()
        '''
        #matplotlib.image.imsave('C:\\Users\\未央\\Desktop\\Graph represented-based band selection\\GCSR\\亲和矩阵.png', L)
        #L = L / L.max()
        #L = 0.5 * (L + L.T)
        spectral = SpectralClustering(n_clusters=K, \
                                      eigen_solver='arpack', \
                                      affinity='precomputed',\
                                      assign_labels='discretize')  #使用sklearn的谱聚类
        spectral.fit(L)
    ...
    def __get_band(self, cluster_result, X):
        ...
        for c in np.unique(cluster_result):
            ...
            distance = np.linalg.norm(X[:, idx[0]] - center, axis=0)  #计算欧氏距离
            band_ = X[:, idx[0]][:, distance.argmin()]
            
            sr = band_.reshape(512,217)
            
            #sr = band_.reshape(145,145)*255
            
            #cv2.imwrite('C:\\Users\\未央\\Desktop\\Graph represented-based band selection\\GCSR\\'+str(yt)+'.png', sr)
            matplotlib.image.imsave('C:\\Users\\未央\\Desktop\\Graph represented-based band selection\\GCSR\\'+str(yt)+'.png', sr)
            #sr1 = Image.fromarray(sr)
