...
class Processor:
    ...
    def prepare_data(self, img_path, gt_path):
        if img_path[-3:] == 'mat':
            ...
            return img_mat.get(img_key[0]).astype('float64'), gt_mat.get(gt_key[0]).astype('int8')   #转换数组数据类型
        else:
            import spectral as spy
    ...
    def get_correct(self, img, gt):
        ...
        """
        gt_1D = gt.reshape(-1)    #变成一行
        index = gt_1D.nonzero()   #统计非零元素所在行/列
        gt_correct = gt_1D[index]
        img_2D = img.reshape(img.shape[0] * img.shape[1], img.shape[2])    #(145*145)*220
        img_correct = img_2D[index]  #提取出gt非零的行作为修正后的数据集
        return img_correct, gt_correct

    def get_tr_tx_index(self, y, test_size=0.9):    #将原始数据按照比例分割为"测试集"和"训练集",test_size为样本占比
        from sklearn.model_selection import train_test_split
        train_index, test_index, y_train_, y_test_ = \
            train_test_split(np.arange(0, y.shape[0]), y, test_size=test_size)
        return train_index, test_index   #返回训练集索引和测试集索引
    ...
    def divide_img_blocks(self, img, gt, block_size=(5, 5)):
        ...
        w_1, w_2 = int((block_size[0] - 1) / 2), int((block_size[1] - 1) / 2)  #w_1, w_2 = (2,2)
