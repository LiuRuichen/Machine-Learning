...
class EGCSR_BS_Ranking:
    ...
    def fit(self, X):
        ...
        # Coef = self.thrC(C, self.ro)
        C[np.diag_indices_from(C)] = 0  #对角线元素为0（避免平凡解）
    ...
    def __get_band(self, C, X):
        ...
        x_axis = np.array([0 for i in range(0,224)])
        for i in range(0,224):
            x_axis[i] = i+1
            
        plt.plot(x_axis,sum_C)
        plt.show()
        ...
        for i in largest_k:
            im = X[:,i]
            sr = im.reshape(512,217)
            matplotlib.image.imsave('C:\\Users\\未央\\Desktop\\Graph represented-based band selection\\GCSR\\ok'+str(yt)+'.png', sr)
            yt = yt + 1
        return selected_band
